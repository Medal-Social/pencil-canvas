...
function NodeIcon({ type }: { type: string }) {
  ...
  switch (type) {
    case 'frame':
      return (<svg {...s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2" /></svg>);
    case 'group':
      return (<svg {...s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="2" width="8" height="8" rx="1" /><rect x="14" y="14" width="8" height="8" rx="1" /><path d="M10 6h4M6 10v4M14 18h-4M18 14v-4" strokeDasharray="2 2" /></svg>);
    case 'text':
      return (<svg {...s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="4 7 4 4 20 4 20 7" /><line x1="12" y1="4" x2="12" y2="20" /><line x1="8" y1="20" x2="16" y2="20" /></svg>);
    case 'ellipse':
      return (<svg {...s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><ellipse cx="12" cy="12" rx="10" ry="8" /></svg>);
    case 'rectangle':
      return (<svg {...s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="5" width="18" height="14" rx="2" /></svg>);
    default:
      return (<svg {...s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="12 2 2 7 12 12 22 7 12 2" /></svg>);
  }
}
...
function LayerRow({ node, depth, hiddenIds, expandedIds, selectedId, searchQuery, onToggle, onToggleExpand, onSelect }: {
  node: ResolvedNode;
  depth: number;
      ...
      >
        <button type="button"
          ...
          <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"
            style={{ transform: (expanded || forceExpand) ? 'rotate(90deg)' : 'rotate(0deg)', transition: 'transform 0.1s' }}>
            <polyline points="9 6 15 12 9 18" />
          </svg>
        </button>
        <NodeIcon type={node.type} />
        <span className={`flex-1 truncate text-[11px] pl-1 ${
        ...
        <button type="button"
          ...
          className={`bg-transparent border-none p-0 w-5 h-5 flex items-center justify-center shrink-0 cursor-pointer transition-colors ${
            hidden ? 'text-[var(--muted-foreground)] opacity-50' : 'text-[var(--muted-foreground)] hover:text-[var(--foreground)]'
          }`}>
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            {hidden
              ? <><line x1="1" y1="1" x2="23" y2="23" /><path d="M6.7 6.7C3.8 8.6 2 12 2 12s4 7 10 7c1.7 0 3.3-.5 4.7-1.3M17.3 17.3C20.2 15.4 22 12 22 12s-4-7-10-7c-1.7 0-3.3.5-4.7 1.3" /></>
              : <><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" /></>
            }
          </svg>
        </button>
      </div>
      {hasChildren && (expanded || forceExpand) && node.children!.map((child) => (
        <LayerRow key={child.id} node={child} depth={depth + 1} hiddenIds={hiddenIds} expandedIds={expandedIds} selectedId={selectedId} searchQuery={searchQuery} onToggle={onToggle} onToggleExpand={onToggleExpand} onSelect={onSelect} />
      ))}
    </>
  );
}
...
const fieldCls = 'bg-[var(--input)] rounded-md h-7 px-2.5 text-[11px] text-[var(--foreground)] font-mono tabular-nums w-full truncate flex items-center';
...
function InspectPanel({ node }: { node: ResolvedNode }) {
  ...
  const gradientFill = typeof node.fill === 'object' && node.fill?.type === 'gradient' ? node.fill : undefined;
  const hasStroke = node.stroke && node.stroke.thickness;
  ...
  return (
    <div className="text-[var(--foreground)]">
          ...
          <div className="px-4 pb-3 space-y-2">
            <div className="flex gap-1">
              {(['horizontal', 'vertical'] as const).map((dir) => (
                <div key={dir} className={`flex-1 py-1 rounded-md text-center text-[10px] ${
                  node.layout === dir
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              {node.justifyContent && <Field label="J" value={node.justifyContent.replace('_', ' ')} />}
      ...
      <div className="px-4 pb-3 grid grid-cols-2 gap-2">
        <Field label="%" value={node.opacity !== undefined ? Math.round(node.opacity * 100) : 100} />
        {node.cornerRadius !== undefined && (
          <Field label="◜" value={typeof node.cornerRadius === 'number' ? node.cornerRadius : node.cornerRadius.join(', ')} />
        )}
      </div>
            ...
            {node.textAlign && (
              <div className="flex gap-0.5">
                {(['left', 'center', 'right'] as const).map((a) => (
                  <div key={a} className={`flex-1 py-1 rounded text-center text-[10px] ${
                    node.textAlign === a
                      ? 'bg-[var(--accent)] text-[var(--foreground)]'
          ...
          <div className="px-4 pb-3">
            {fillColor && !fillColor.startsWith('$') ? (
              <div className="flex items-center gap-2">
                <span className="w-6 h-6 rounded-md border border-[var(--sidebar-border)] shrink-0" style={{ background: fillColor }} />
                <div className={`${fieldCls} flex-1`}>{fillColor}</div>
                <span className="text-[10px] text-[var(--muted-foreground)] tabular-nums">100 %</span>
              </div>
            ) : fillColor?.startsWith('$') ? (
              <div className="flex items-center gap-2">
                <span className="w-6 h-6 rounded-md border border-dashed border-[var(--border)] shrink-0 flex items-center justify-center text-[10px] text-[var(--muted-foreground)]">$</span>
                <div className={`${fieldCls} flex-1 text-[var(--muted-foreground)]`}>{fillColor}</div>
              </div>
            ) : gradientFill ? (
              <div className="flex items-center gap-2">
                <span className="w-6 h-6 rounded-md border border-[var(--sidebar-border)] shrink-0" style={{
                  background: `linear-gradient(${gradientFill.rotation ?? 0}deg, ${gradientFill.colors?.map((c) => c.color).join(', ') ?? '#333'})`,
                }} />
                <div className={`${fieldCls} flex-1`}>{gradientFill.gradientType ?? 'linear'} gradient</div>
              </div>
            ) : null}
          </div>
        </>
      ...
      {hasStroke && (
        <>
          ...
          <div className="px-4 pb-3 space-y-2">
            {node.stroke!.fill && (
              <div className="flex items-center gap-2">
                <span className="w-6 h-6 rounded-md border border-[var(--sidebar-border)] shrink-0" style={{ background: node.stroke!.fill }} />
            ...
            <div className="grid grid-cols-2 gap-2">
              {node.stroke!.align && (
                <div className={fieldCls}>{node.stroke!.align}</div>
              )}
          ...
          <div className="px-4 pb-3 space-y-2">
            {effects.map((e, i) => (
              <div key={i} className="flex items-center gap-2">
                {e.color && (
                  <span className="w-4 h-4 rounded border border-[var(--sidebar-border)] shrink-0" style={{ background: e.color }} />
                )}
                <div className="flex-1">
                  <div className="text-[11px]">{e.type === 'shadow' ? `${e.shadowType ?? 'outer'} shadow` : e.type}</div>
                  <div className="text-[10px] text-[var(--muted-foreground)]">blur: {e.blur ?? 0}{e.spread ? ` spread: ${e.spread}` : ''}</div>
                </div>
              </div>
            ))}
          </div>
...
function countVisibleLayers(nodes: ResolvedNode[], hiddenIds: Set<string>): number {
  let count = 0;
  function walk(n: ResolvedNode) { if (!hiddenIds.has(n.id)) { count++; n.children?.forEach(walk); } }
  nodes.forEach(walk);
  return count;
}
...
function countTotalLayers(nodes: ResolvedNode[]): number {
  let count = 0;
  function walk(n: ResolvedNode) { count++; n.children?.forEach(walk); }
  nodes.forEach(walk);
  return count;
}
...
function collectAllIds(nodes: ResolvedNode[]): Set<string> {
  ...
  function walk(n: ResolvedNode) { if (n.children && n.children.length > 0) ids.add(n.id); n.children?.forEach(walk); }
  nodes.forEach(walk);
  return ids;
}
...
export function PenViewer({
  data, width = 1440, height = 900, className, style, showToolbar = true, variables,
}: PenViewerProps) {
  ...
  const hoveredNode = useMemo(
    () => hoveredNodeId && hoveredNodeId !== selectedNodeId ? findNodeById(resolved, hoveredNodeId) : undefined,
  ...
  const nodeIndex = useMemo(() => {
    ...
    function walk(n: ResolvedNode) { map.set(n.id, n); n.children?.forEach(walk); }
    resolved.forEach(walk);
    return map;
  }, [resolved]);

  const visibleCount = useMemo(() => countVisibleLayers(resolved, hiddenIds), [resolved, hiddenIds]);
  ...
  const fitToScreen = useCallback(() => { setZoom(1); setPanX(0); setPanY(0); }, []);
  ...
  const doExport = useCallback((format: ExportFormat, scale: number = 2) => {
  ...
  }, [contentW, contentH, pad, selectedNode, showToast]);
  ...
  return (
    <div ref={containerRef} className={`bg-[var(--background)] ${className ?? ''}`} style={wrapStyle}>
      {/* Left sidebar */}
      <div className="shrink-0 overflow-hidden bg-[var(--sidebar)] flex flex-col transition-[width] duration-150 ease-in-out"
        style={{ width: sidebarOpen ? 240 : 0, borderRight: sidebarOpen ? '1px solid var(--sidebar-border)' : 'none' }}>
        {/* Tab bar */}
        <div className="flex items-center border-b border-[var(--sidebar-border)] overflow-x-auto" style={{ minWidth: 240 }}>
          ...
          <div className="flex-1 overflow-y-auto flex flex-col" style={{ minWidth: 240 }}>
            ...
            <div className="px-2 pt-2 pb-1">
              <div className="flex items-center gap-2 h-8 rounded-md bg-[var(--input)] px-2.5 text-[var(--muted-foreground)] focus-within:ring-1 focus-within:ring-[var(--ring)]">
                ...
                <input ref={searchRef} type="text" placeholder="Search layers..."
                ...
                />
                {layerSearch && (
                  <button type="button" onClick={() => setLayerSearch('')}
            ...
            <div className="flex items-center justify-between px-3 py-1.5 bg-[var(--sidebar-accent)]">
              <span className="text-[10px] text-[var(--muted-foreground)]">
                {visibleCount === totalCount ? `${totalCount} layers` : `${visibleCount} / ${totalCount} visible`}
              </span>
              {hiddenIds.size > 0 && (
                <button type="button" onClick={() => setHiddenIds(new Set())}
            ...
            <div className="flex-1 overflow-y-auto py-0.5">
              {resolved.map((node) => (
                <LayerRow key={node.id} node={node} depth={0} hiddenIds={hiddenIds} expandedIds={expandedIds} selectedId={selectedNodeId} searchQuery={searchLower} onToggle={toggleLayer} onToggleExpand={toggleExpand} onSelect={(id) => { selectNode(id); }} />
              ))}
            </div>
          </div>
        )}
        ...
        {sidebarTab === 'components' && (
          <div className="flex-1 overflow-y-auto" style={{ minWidth: 240 }}>
            <div className="px-2 py-1.5 border-b border-[var(--sidebar-border)]">
              <span className="text-[10px] text-[var(--muted-foreground)]">{namedFrames.length} components</span>
            </div>
            <div className="py-0.5">
              {namedFrames.map((frame) => (
                <button key={frame.id} type="button"
                  onClick={() => { setSelectedNodeId(frame.id); }}
                  ...
                  <span className="text-[9px] text-[var(--muted-foreground)]">{Math.round(frame.resolvedWidth)}&times;{Math.round(frame.resolvedHeight)}</span>
                </button>
              ))}
              {namedFrames.length === 0 && (
                <div className="p-4 text-[var(--muted-foreground)] text-[11px] text-center">No named frames found</div>
              )}
            </div>
          </div>
        )}
        ...
        {sidebarTab === 'libraries' && variables && (
          <div className="flex-1 overflow-y-auto" style={{ minWidth: 240 }}>
            <div className="px-2 py-1.5 border-b border-[var(--sidebar-border)]">
              <span className="text-[10px] text-[var(--muted-foreground)]">{Object.keys(variables).length} tokens</span>
            </div>
            <div style={{ padding: '4px 0' }}>
              {Object.entries(variables).map(([key, value]) => (
                <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 10px' }}
                  ...
                  {value.startsWith('#') && (
                    <span style={{ width: 14, height: 14, borderRadius: 4, background: value, border: '1px solid #333', flexShrink: 0 }} />
                  )}
      ...
      <div className="flex-1 flex flex-col relative min-w-0">
        ...
        >
          <div style={{
            ...
            <svg ref={svgRef} viewBox={vb} width="100%" height="100%"
              ...
              {resolved.map((node) => !hiddenIds.has(node.id) && <RenderNode key={node.id} node={node} hiddenIds={hiddenIds} variables={variables} nodeIndex={nodeIndex} />)}
                  ...
                  return (
                    <g key={node.id}>
                      <rect
                        ...
                        width={Math.max(0, node.resolvedWidth)} height={Math.max(0, node.resolvedHeight)}
              ...
              {selectedNode && (
                <g data-selection-overlay="true" style={{ pointerEvents: 'none' }}>
                  <rect
                    x={selectedNode.resolvedX - 1} y={selectedNode.resolvedY - 1}
                    width={selectedNode.resolvedWidth + 2} height={selectedNode.resolvedHeight + 2}
                    fill="none" stroke="#4A9FD8" strokeWidth={1.5}
                    strokeDasharray="4 2"
                  />
                  {/* Dimension label */}
                  <rect
                    x={selectedNode.resolvedX} y={selectedNode.resolvedY + selectedNode.resolvedHeight + 4}
                    width={Math.max(60, String(`${Math.round(selectedNode.resolvedWidth)} × ${Math.round(selectedNode.resolvedHeight)}`).length * 6 + 12)}
                    height={16} rx={3} fill="#4A9FD8"
                  />
                  <text
                    x={selectedNode.resolvedX + 6}
                    y={selectedNode.resolvedY + selectedNode.resolvedHeight + 15}
                    fill="#fff" fontSize={9} fontFamily="monospace"
                  >
                    {Math.round(selectedNode.resolvedWidth)} &times; {Math.round(selectedNode.resolvedHeight)}
                  </text>
                </g>
              )}
              ...
              {hoveredNode && (
                <g data-selection-overlay="true" style={{ pointerEvents: 'none' }}>
                  {/* Hover outline */}
                  <rect
                    x={hoveredNode.resolvedX - 0.5} y={hoveredNode.resolvedY - 0.5}
                    width={hoveredNode.resolvedWidth + 1} height={hoveredNode.resolvedHeight + 1}
                    fill="none" stroke="#4A9FD8" strokeWidth={0.75} opacity={0.6}
                  />
                  ...
                  {selectedNode && (() => {
                    ...
                    // Horizontal distance
                    let hDist = 0;
                    let hx1 = 0, hx2 = 0, hy = 0;
                    if (hRight <= s.resolvedX) {
                      hDist = s.resolvedX - hRight; hx1 = hRight; hx2 = s.resolvedX; hy = (sMidY + hMidY) / 2;
                    } else if (h.resolvedX >= sRight) {
                      hDist = h.resolvedX - sRight; hx1 = sRight; hx2 = h.resolvedX; hy = (sMidY + hMidY) / 2;
                    }
                    if (hDist > 0) {
                      const label = String(Math.round(hDist));
                      lines.push(
                        <line key="h-line" x1={hx1} y1={hy} x2={hx2} y2={hy} stroke="#E74C3C" strokeWidth={0.75} />,
                        <rect key="h-bg" x={(hx1 + hx2) / 2 - label.length * 3 - 4} y={hy - 8} width={label.length * 6 + 8} height={14} rx={3} fill="#E74C3C" />,
                        <text key="h-txt" x={(hx1 + hx2) / 2} y={hy + 1} fill="#fff" fontSize={8} fontFamily="monospace" textAnchor="middle">{label}</text>,
                      );
                    }

                    // Vertical distance
                    let vDist = 0;
                    let vy1 = 0, vy2 = 0, vx = 0;
                    if (hBottom <= s.resolvedY) {
                      vDist = s.resolvedY - hBottom; vy1 = hBottom; vy2 = s.resolvedY; vx = (sMidX + hMidX) / 2;
                    } else if (h.resolvedY >= sBottom) {
                      vDist = h.resolvedY - sBottom; vy1 = sBottom; vy2 = h.resolvedY; vx = (sMidX + hMidX) / 2;
                    }
                    if (vDist > 0) {
                      const label = String(Math.round(vDist));
                      lines.push(
                        <line key="v-line" x1={vx} y1={vy1} x2={vx} y2={vy2} stroke="#E74C3C" strokeWidth={0.75} />,
                        <rect key="v-bg" x={vx + 4} y={(vy1 + vy2) / 2 - 7} width={label.length * 6 + 8} height={14} rx={3} fill="#E74C3C" />,
                        <text key="v-txt" x={vx + 4 + label.length * 3 + 4} y={(vy1 + vy2) / 2 + 2} fill="#fff" fontSize={8} fontFamily="monospace" textAnchor="middle">{label}</text>,
                      );
                    }
        ...
        {showToolbar && (
          <div style={{
            ...
            padding: '4px 4px', boxShadow: '0 8px 24px rgba(0,0,0,0.4)', zIndex: 10,
          }}>
            {/* Sidebar toggle */}
            <button type="button" onClick={() => setSidebarTab(sidebarTab ? null : 'layers')}
              style={{ ...btnStyle, color: sidebarOpen ? '#fff' : '#999', background: sidebarOpen ? '#333' : 'none' }}
              onMouseEnter={hoverIn} onMouseLeave={hoverOut} title="Toggle sidebar">
            ...
            <div style={{ position: 'relative' }}>
              ...
              {showZoomMenu && (
                <div style={{ position: 'absolute', bottom: 40, left: '50%', transform: 'translateX(-50%)', background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 10, padding: '4px', minWidth: 160, boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }}>
                  {ZOOM_PRESETS.map((p) => (
                    <button key={p.label} type="button" onClick={() => setZoomTo(p.value)}
                      style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', padding: '8px 12px', background: 'none', border: 'none', color: '#ccc', fontSize: 13, cursor: 'pointer', borderRadius: 6, textAlign: 'left' }}
                      onMouseEnter={(e) => { e.currentTarget.style.background = '#2a2a2a'; }}
                      onMouseLeave={(e) => { e.currentTarget.style.background = 'none'; }}>
                      <span>{p.label}</span>
                      {p.shortcut && <span style={{ fontSize: 11, color: '#666', background: '#2a2a2a', padding: '1px 6px', borderRadius: 4, fontFamily: 'monospace' }}>{p.shortcut}</span>}
                    </button>
                  ))}
                </div>
              )}
            ...
            {/* Right panel toggle */}
            <button type="button" onClick={() => setShowRightPanel(!showRightPanel)}
              style={{ ...btnStyle, color: showRightPanel ? '#fff' : '#999', background: showRightPanel ? '#333' : 'none' }}
              onMouseEnter={hoverIn} onMouseLeave={hoverOut}
        ...
        {rightPanelVisible && (
          <div className="absolute inset-y-0 right-0 w-72 bg-[var(--sidebar)] border-l border-[var(--sidebar-border)] flex flex-col z-10">
            <div className="flex-1 overflow-y-auto">
              ...
              {/* Variables */}
              {variables && typeof selectedNode.fill === 'string' && selectedNode.fill.startsWith('$') && (
                <>
                  <SectionHeader title="Variables" />
                  <div className="px-4 pb-3">
                    {(() => {
                      const resolved = variables[selectedNode.fill] ?? variables[selectedNode.fill.slice(1)];
                      return resolved ? (
                        <div className="flex items-center gap-2">
                          <span className="w-4 h-4 rounded border border-[var(--sidebar-border)] shrink-0" style={{ background: resolved }} />
                        ...
                        <div className="text-[11px] text-[var(--muted-foreground)]">{selectedNode.fill} (unresolved)</div>
                      );
                    })()}
                  </div>
                </>
        ...
        {toast && (
          <div style={{
            ...
            padding: '10px 16px', boxShadow: '0 8px 24px rgba(0,0,0,0.4)',
            zIndex: 20, display: 'flex', alignItems: 'center', gap: 8,
            fontSize: 13, color: '#ccc', whiteSpace: 'nowrap',
          }}>
...
const btnStyle: React.CSSProperties = {
  ...
  width: 32, height: 32, borderRadius: 8, padding: 0, transition: 'all 0.1s',
};
